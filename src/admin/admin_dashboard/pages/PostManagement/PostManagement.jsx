        ...
        <div className="post_management_wrapper">
          ...
          <div className="post_management_form">
            ...
            <FormControl fullWidth sx={{ py: 1 }}>
              ...
              />
              {error.post_title && (
                <Alert color="error">{error.post_title}</Alert>
              )}
            </FormControl>
            <FormControl fullWidth sx={{ py: 1 }}>
              ...
              />
              {error.post_tags && (
                <Alert color="error">{error.post_tags}</Alert>
              )}
            </FormControl>
            <FormControl fullWidth sx={{ py: 1 }}>
              <Form.Label htmlFor="Author">Post Published</Form.Label>
              <Form.Control
                onChange={(e) => setPostPublished(e.target.value)}
              ...
              {error.post_published && (
                <Alert color="error">{error.post_published}</Alert>
              )}
            </FormControl>
            <FormControl fullWidth sx={{ py: 1 }}>
            ...
            <FormControl fullWidth sx={{ py: 1 }}>
              ...
              {error.post_thumbnail && (
                <Alert color="error">{error.post_thumbnail}</Alert>
              )}
            </FormControl>
            <FormControl fullWidth sx={{ py: 1 }}>
              ...
              >
                ...
                {AllCategory.map((items) => (
                  <option key={items.category_id} value={items.category_id}>
                    {items.category_name}
                  </option>
                ))}
              </Form.Select>
              {error.post_category && (
                <Alert color="error">{error.post_category}</Alert>
              )}
            </FormControl>
            <FormControl fullWidth sx={{ py: 1 }}>
              ...
              {error.post_description && (
                <Alert color="error">{error.post_description}</Alert>
              )}
            </FormControl>
            <FormControl>
              {isEdit ? (
                ...
                {AllPost.map((items, index) => (
                  <tr key={index}>
                    <td align="center" style={{ textTransform: "uppercase" }}>
                      <Avatar
                        src={`http://localhost:5173/server/public/uploads/blog_img/${items.post_thumbnail}`}
                      />
                    </td>
                    <td align="center">
                      {items.post_title && items.post_title.slice(0, 5)}
                    </td>
                    <td align="center">
                      {items.post_tags &&
                        items.post_tags
                    ...
                    <td align="center">
                      {items.post_published &&
                        items.post_published.split("T")[0]}
                    </td>
                    <td align="center">{items.post_author}</td>
                    <td align="center">
                      {AllCategory.filter(
                        (ct) => ct.category_id == items.post_category
                      ).map((ctn) => ctn.category_name)}
                    </td>
                    {/* <div dangerouslySetInnerHTML={{ __html: post_description }} /> */}
                    <td align="center">
                      <div
                        dangerouslySetInnerHTML={{
                          __html:
                            ...
                            items.post_description.slice(0, 20),
                        }}
                      />
                    </td>
                    <td align="center">
                      <IconButton
                        color="success"
                        onClick={() => edit_post(items.post_id)}
                      >
                        <Edit />
                      </IconButton>
                      <IconButton
                        color="error"
                        onClick={() => delete_post(items.post_id)}
                      >
                        <Delete />
                      </IconButton>
                    </td>
